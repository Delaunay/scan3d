...
double randAToB(double a, double b) { return a + (rand() / (RAND_MAX / (b - a))); }
...
int Triangulation::matrixCorr(Mat &pointsLut, Mat &pointsCorr, const Mat &lutSrc,
    ...
    for(int r = 0; r < lutSrc_r; r++) {
        for(int c = 0; c < lutSrc_c; c++) {
            ...
            if(n == 10000)
                printf("(%f, %f) -> (%f, %f) \n", pointsLut.at<double>(0, n),
                       pointsLut.at<double>(1, n), pointsCorr.at<double>(0, n),
                       pointsCorr.at<double>(1, n));
            n++;
        }
    }
...
void Triangulation::triangulate(const Mat &lutCam, const Mat &lutProj) {
    ...
    if(TR_CAM) {
        /* Cam -> Proj */
        size = lut2corr(lutCam, internes_cam, distCoeffs_cam, pointsUndLut, lutProj, internes_proj,
                        distCoeffs_proj, pointsUndCorr);

        point4D = Mat::zeros(4, size, CV_64F);
        triangulatePoints(poseMatrix_cam, poseMatrix_proj, pointsUndLut, pointsUndCorr, point4D);
    } else {
        /* Proj -> Cam */
        size = lut2corr(lutProj, internes_proj, distCoeffs_proj, pointsUndLut, lutCam, internes_cam,
                        distCoeffs_cam, pointsUndCorr);
...
void Triangulation::setPathT(int idx, const string &path, const char *filename) {
    ...
    switch(idx) {
    case IDX_TR_MASK:
        fn_tr_mask = strdup(newfilename.c_str());
        break;
    case IDX_TR_DATA:
        fn_tr_data = strdup(newfilename.c_str());
        break;
    case IDX_TR_PARC:
        fn_tr_parc = strdup(newfilename.c_str());
        break;
    case IDX_TR_PARP:
        fn_tr_parp = strdup(newfilename.c_str());
        break;
    default:
        printf("setPathT: code %d inconnu!!!!!!!!!\n", idx);
        break;
    }
}
