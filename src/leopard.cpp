...
typedef struct {
    unsigned int byte;  // 0..nb-1 (c'est en fait un unsigned long, donc max bits =
                        // 255*64=16384 bits
...
std::vector<cv::Mat> leopard::readImagesFromCam(const std::vector<Mat> &cam, int from, int to) {
...
void leopard::computeMask(int cam, const std::vector<cv::Mat> &img, int nb, double seuil,
    ...
    for(int j = 0; j < nr * nc; j++) {
        psumL[j] = psum2L[j] = psumH[j] = psum2H[j] = pcountL[j] = pcountH[j] = 0;
    }
    ...
    for(int i = 0; i < nb; i++) {
        ...
        for(int j = 0; j < nr * nc; j++) {
            if(p[j] < pmean[j]) {
                psumL[j] += p[j];
                psum2L[j] += (int)p[j] * p[j];
                pcountL[j] += 1;
            } else if(p[j] > pmean[j]) {
                psumH[j] += p[j];
                psum2H[j] += (int)p[j] * p[j];
                pcountH[j] += 1;
            }
        }
    }

    // on veut comparer la deviation standard de chaque cote avec la distance
    // entre les moyennes ratio (m2-m1)/std si c'est <=1 unimodal si >>1 bimodal
    cv::Mat bimod;
    ...
    for(int j = 0; j < nr * nc; j++) {
        ...
        double v = (mH - mL) / (2 * sqrt(stdL + stdH) +
                                bias); // le +5 est pour limiter l'effet lorsque mH-mL -> 0
    ...
    if(cam) {
        ...
        for(int y = ymin; y <= ymax; y += step)
            for(int x = xmin; x <= xmax; x += step)
                maskCam[y * wc + x] = pmask[y * wc + x];
        // ajuste le mask pour l'image
        for(int j = 0; j < wc * hc; j++)
            if(maskCam[j] == 0)
                pmask[j] = 0;
        if(n > 0 && n != nb) {
            printf("Cam et Proj : pas le meme nombre d'images!!!!\n");
            exit(0);
        }
        n = nb;
    } else {
...
void leopard::computeCodes(int cam, int type, cv::Mat *img) {
    ...
    if(cam) {
        ...
        if(!maskCam) {
            printf("cam: No mask to compute codes!\n");
            exit(0);
        }
        pmask = maskCam;
    } else {
        ...
        if(!maskProj) {
            printf("proj: No mask to compute codes!\n");
            exit(0);
        }
        pmask = maskProj;
        printf("*** WP=%d HP=%d\n", wp, hp);
    ...
    if(type == LEOPARD_SIMPLE) {
        ...
        for(int j = 0; j < w * h; j++) {
            if(pmask[j] == 0)
                continue; // tous les codes en dehors du mask sont 0
            count++;
            for(int i = 0; i < n; i++) {
                unsigned char *p = img[i].data;
                unsigned short v = (p[j] << 8) | 0x80;
                if(v > pmean[j] || (v == pmean[j] && (rand() & 1)))
                    mpz_setbit(code[j], i);
                if(v == pmean[j]) {
                    ambigu++;
                } // ne sert plus a rien
        ...
        for(int i = 0; i < n; i++) {
            ...
            for(int j = i + 1; j < n; j++) {
                ...
                for(int k = 0; k < w * h; k++) {
                    if(pmask[k] == 0)
                        continue;
                    if(pi[k] > pj[k] || (pi[k] == pj[k] && (nbk & 1)))
                        mpz_setbit(code[k], nbk);
                }
                nbk++;
            }
        }
        ...
        for(int j = 0; j < w * h; j++) {
            ...
            for(int i = 0; i < n; i++) {
                ...
                if(v == pmean[j]) {
                  ...
                  //                if( j==200*w+300 ) { printf("image[%2d] = %d = %d,
                  //                mean=%d\n",i,p[j],v,pmean[j]);
...
void leopard::prepareMatch() {
    ...
    // initialise avec le cout le plus eleve pour avoir un match valide mais si
    // inactif
    for(int i = 0; i < wc * hc; i++) {
        matchCam[i].idx   = 0;
        matchCam[i].cost  = 9999;
        matchCam[i].subpx = 0.0;
        matchCam[i].subpy = 0.0;
        matchCam[i].mix   = 0;
    }
    for(int i = 0; i < wp * hp; i++) {
        matchProj[i].idx   = 0;
        matchProj[i].cost  = 9999;
        matchProj[i].subpx = 0.0;
        matchProj[i].subpy = 0.0;
        matchProj[i].mix   = 0;
    }
}
...
#ifdef USE_GMP
void leopard::unSousPixels(int i) { printf("NA1\n"); }
#else
...
void leopard::unSousPixels(int i) {
    ...
    int jy = j / wp;
    int jx = j % wp;
    if((jx < wDecal) || (jx >= wp - wDecal))
        return;
    if((jy < wDecal) || (jy >= hp - wDecal))
        return;
    // printf("i = %d, j = %d\n", i, j);
...
int leopard::lsh(int dir, unsigned long *codeA, minfo *matchA, unsigned char *maskA, int wa, int ha,
    ...
    // log << "lsh " << nv << " bits, allocate "<< ((1<<nv)*sizeof(int)) << "
    // bytes"<<warn;

    if(vote == NULL)
        vote = (int *)malloc(nbvote * sizeof(int));
    int *q = vote;
    ...
    for(int i = 0; i < nv; i++) {
        ...
        // log << "bit " << i << " byte="<<(int)bm[i].byte<<",
        // mask="<<(int)bm[i].mask<<", vmask="<<bm[i].vmask<<info;
    }
    ...
    for(int i = start; i != stop; i += step) {
        ...
        mpz_t *p = codeA + i;
        for(int k = 0; k < nv; k++)
            if(mpz_tstbit(*p, bm[k].byte))
                hash |= bm[k].vmask;
#else
        p          = codeA + i * nb;
        for(int k = 0; k < nv; k++) {
            if(p[bm[k].byte] & bm[k].mask)
                hash |= bm[k].vmask;
        }
#endif
        if(vote[hash] >= 0)
        ...
        // vote[hash]=i;
        // better option??? if there is already a vote, replace if we have a worse
        // best match
        if(vote[hash] < 0 || matchA[i].cost > matchA[vote[hash]].cost)
            vote[hash] = i;
#if 0
            else{
                for(int g=1;g<10;g++) {
    ...
    printf("time=%12.6f  redox=%10d A=%4d B=%3d vide=%3d%% collisions=%3d%% "
           "vide= %d (%d,%d)\n",
           now, redox, nbmatchA, nbmatchB, holes / (nbvote / 100), collision / (count / 100), vide,
           nbvote, count);
    return (vide);
}
...
void leopard::shiftCodes(int shift, unsigned long *codes, int w, int h) {
    ...
    for(int i = 0; i < w * h; i++) {
        ...
        for(int j = 0; j < nb; j++) {
            if(j < nb - 1) {
                codes[i * nb + j] =
                    (codes[i * nb + j] >> shift) | (codes[i * nb + j + 1] << (s - shift));
            } else {
                s                 = nbb % 64;
    ...
    //            if(j > 0) {
    //                codes[i*nb+j] = (codes[i*nb+j] >> shift) | (codes[i*nb+j-1]
    //                << (s - shift));
    //            }
    //            else {
    //                codes[i*nb+j] = (codes[i*nb+j] >> shift) | (mem << (s -
    //                shift));
    //            }
    //        s = 64;
    //        }
...
int leopard::doShiftCodes() {
    ...
    for(int i = 0; i < nbb; i++) {
        ...
        for(int k = 0; k < 2; k++) {
            srand(1656 + k);
            videCam += lsh(1, codeCam, matchCam, maskCam, wc, hc, codeProj, matchProj, maskProj, wp,
                           hp, -1, 0);
            srand(1678 + k);
            videProj += lsh(1, codeProj, matchProj, maskProj, wp, hp, codeCam, matchCam, maskCam,
                            wc, hc, -1, 0);
        }
    ...
    for(y = 0; y < h; y++)
        for(x = 0; x < w; x++) {
            ...
            if(mask[i] == 0) {
                ...
                lut.at<Vec3s>(y, x) =
                    Vec3s((cc * 65535) / nbb, (yy * 65535 + dyy) / hh, (xx * 65535 + dxx) / ww);
            }
        }
}
...
void leopard::setPathL(int idx, std::string path, const char *filename) {
    ...
    switch(idx) {
    case IDX_SCAN_MASKC:
        fn_scan_maskc = strdup(newfilename.c_str());
        break;
    case IDX_SCAN_MEANC:
        fn_scan_meanc = strdup(newfilename.c_str());
        break;
    case IDX_SCAN_MASKP:
        fn_scan_maskp = strdup(newfilename.c_str());
        break;
    case IDX_SCAN_MEANP:
        fn_scan_meanp = strdup(newfilename.c_str());
        break;
    default:
        printf("setPathL: code %d inconnu!!!!!!!!!\n", idx);
        break;
    }
}
